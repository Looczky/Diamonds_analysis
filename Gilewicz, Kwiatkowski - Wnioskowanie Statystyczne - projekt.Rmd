...
```{r}
model_1_xy <- lm(x ~ y, diamonds)
summary(model_1_xy)
plot_1_xy <- ggplot(diamonds, aes(x, y)) +
              geom_point() +
              geom_smooth(method='lm', formula='y~x') +
              labs(title='Wykresy zależności między wymiarami diamentów')

model_1_xz <- lm(x ~ z, diamonds)
summary(model_1_xz)
plot_1_xz <- ggplot(diamonds, aes(x, z)) +
              geom_point() +
              geom_smooth(method='lm', formula='y~x')

model_1_yz <- lm(y ~ z, diamonds)
summary(model_1_yz)
plot_1_yz <- ggplot(diamonds, aes(y, z)) +
              geom_point() +
              geom_smooth(method='lm', formula='y~x')

grid.arrange(plot_1_xy, plot_1_xz, plot_1_yz, nrow=3)
```
...
```{r}
model_2 <- lm(carat ~ volume, diamonds)
summary(model_2)
```
...
```{r}
model_H3 <- lm(price ~ carat, diamonds)
summary(model_H3)
```
...
```{r}
plot_4_price <- ggplot(diamonds, aes(color, price, fill=color)) +
  labs(title='Wykres pudełkowy ceny w zależności od koloru') +
  xlab('Kolor') +
  ylab('Cena') +
  geom_boxplot()

plot_4_pricecarat <- ggplot(diamonds,aes(color, price_carat, fill=color)) +
  labs(title='Wykres pudełkowy ceny za karat w zależności od koloru') +
  xlab('Kolor') +
  ylab('Dolar za karat') +
  geom_boxplot()

grid.arrange(plot_4_price, plot_4_pricecarat, nrow=2)
```
...
```{r}
plot_5_price <- ggplot(diamonds, aes(cut, price, fill=cut)) +
  labs(title='Wykres pudełkowy ceny w zależności od jakości szlifu') +
  xlab('Jakość szlifu') +
  ylab('Cena') +
  geom_boxplot()

plot_5_pricecarat <- ggplot(diamonds, aes(cut, price_carat, fill=cut)) +
  labs(title='Wykres pudełkowy ceny za karat w zależności od jakości szlifu') +
  xlab('Jakość szlifu') +
  ylab('Dolar na karat') +
  geom_boxplot()

grid.arrange(plot_5_price, plot_5_pricecarat, nrow=2)
```
...
```{r}
model_H1_color <- aov(carat ~ color, diamonds)
print(summary.lm(model_H1_color))

model_H1_clarity <- aov(carat ~ clarity, diamonds)
print(summary.lm(model_H1_clarity))
...
```{r}
model_H1_both <- aov(carat ~ color + clarity, diamonds)
print(summary.lm(model_H1_both))
```
...
```{r}
model_H2_cut <- aov(volume ~ cut, diamonds)
print(summary.lm(model_H2_cut))
```
...
```{r}
model_H2_carat_cut <- aov(carat ~ cut, diamonds)
print(summary.lm(model_H2_carat_cut))
```
...
```{r}
anova_H2_carat_cut <- aov(carat ~ cut, diamonds)
summary.lm(anova_H2_carat_cut)
```
...
```{r}
anova_H2_color <- aov(volume ~ color, diamonds)
summary.lm(anova_H2_color)
```
...
anova_H2_clarity <- aov(volume ~ clarity, diamonds)
summary.lm(anova_H2_clarity)
```
...
```{r}
anova_H2_1 <- aov(volume ~ color + cut, diamonds)
anova_H2_2 <- aov(volume ~ clarity + cut, diamonds)
anova_H2_3 <- aov(volume ~ color + clarity, diamonds)
anova_H2_4 <- aov(volume ~ color + clarity + cut, diamonds)

summary.lm(anova_H2_1)
summary.lm(anova_H2_2)
summary.lm(anova_H2_3)
summary.lm(anova_H2_4)
```
...
model_H3 <- lm(depth ~ table, diamonds)
summary.lm(model_H3)
```
...
```{r}
plot_6_color <- ggplot(diamonds, aes(table, depth, color=color)) +
  labs(title='Zależność zmiennej depth od table', subtitle='Uwzględniony podział na kolor') +
  geom_point()

plot_6_cut <- ggplot(diamonds, aes(table, depth, color=cut)) +
  labs(subtitle='Uwzględniony podział na jakość szlifu') +
  geom_point()

plot_6_clarity <- ggplot(diamonds, aes(table, depth, color=clarity)) +
  labs(subtitle='Uwzględniony podział na poziom przejrzystości') +
  geom_point()

grid.arrange(plot_6_color, plot_6_cut, plot_6_clarity, nrow=3)
```
...
```{r}
model_H3_cut <- lm(depth ~ table + cut, diamonds)
summary.lm(model_H3_cut)
```

Zmienna `cut` pozwala na objaśnienie \~26% wariancji. Sprawdźmy, czy zaimplementowanie interakcji będzie zasadne.

```{r}
model_H3_cut_2 <- lm(depth ~ table * cut, diamonds)
summary.lm(model_H3_cut_2)
```
...
```{r}
model_H3_color <- lm(depth ~ table + color, diamonds)
summary.lm(model_H3_color)

model_H3_clarity <- lm(depth ~ table + clarity, diamonds)
summary.lm(model_H3_clarity)
```
