...
```
Widać pewną zależność ceny od koloru, jednak brak jakiejkolwiek zależności dla ceny za karat od koloru.
Sprawdźmy jak wygląda cena przy poszczególnych jakościach szlifu.
...
ggplot(diamonds, aes(cut, volume, fill=cut)) +
  labs(title='Wykres pudełkowy jakości szlifu w zależności od koloru') +
  xlab('Jakość szlifu') +
  ylab('Rozmiar') +
  geom_boxplot()
```
...
```{r}
model_1.color <- aov(carat ~ color, diamonds)
summary.lm(model_1.color)
```
